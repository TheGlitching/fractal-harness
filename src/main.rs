...
#[command(
    name = "fractal",
    about = "A persistent task tree executed by ephemeral agents"
)]
struct Cli {
...
enum Commands {
    ...
    Verify,
    /// Show what a node actually changed, from git rather than from its claims
    NodeDiff {
        /// Summary only, no patch body
        #[arg(long)]
        stat: bool,
        /// Node id (omit to list all node commits)
    ...
    match cli.command {
        ...
        Commands::Retry { node_id } => {
            ...
            match s.retry(&node_id) {
                Ok(n) => {
                    println!(
                        "retried {} nodes from '{}' and below — run `fractal run -p {}` to resume",
                        n,
                        node_id,
                        project.display()
                    );
        ...
        Commands::Export { output } => {
            ...
            match s.export_workspace(&target) {
                Ok(count) => {
                    println!(
                        "Successfully exported {} unified code artifacts to: {}",
                        count,
                        target.display()
                    );
                }
        ...
        Commands::Split { subtasks, node } => {
            ...
            let subtasks_val: serde_json::Value =
            ...
            let _ = std::fs::write(&decision_file, serde_json::to_string(&decision).unwrap());
            println!(
                "Node '{}' marked SPLIT into {} subtasks.",
                target_node_id,
                subtasks_arr.as_array().map(|a| a.len()).unwrap_or(0)
            );
        }
        Commands::Escalate {
            assumption,
        ...
        Commands::NodeDiff { stat, node_id } => {
            let mut args: Vec<String> = Vec::new();
            if stat {
                args.push("--stat".to_string());
            }
            // The script's first positional is the node id; keep the slot so the
            // project path lands in the second positional it expects.
            args.push(node_id.unwrap_or_default());
            args.push(project.to_string_lossy().into_owned());
            run_embedded_script(NODE_DIFF_SCRIPT, &args);
        }
        Commands::IntegrateCheck => {
            run_embedded_script(
...
const NODE_DIFF_SCRIPT: &str = include_str!("../bin/fractal-node-diff.sh");
const INTEGRATE_CHECK_SCRIPT: &str = include_str!("../bin/fractal-integrate-check.sh");

/// Run one of the helper scripts bundled into the binary. They are shell
/// programs, not library code, so they are executed as such rather than ported.
fn run_embedded_script(script: &str, args: &[String]) -> ! {
    ...
    match status {
        ...
        Err(e) => {
            eprintln!("fractal: could not run embedded helper script: {e}");
            std::process::exit(1);
        }
    }
}
...
fn pick_model(model_override: Option<&str>, interactive: bool) -> String {
    ...
    let header = format!(
        " Choose model (↑↓ move, enter/space confirm, q quit)  [{} models] ",
        models.len()
    );
    ...
    loop {
        if let Ok(Event::Key(key)) = event::read() {
            ...
            match key.code {
                ...
                KeyCode::Down if selected + 1 < models.len() => {
                    selected += 1;
                }
                KeyCode::PageUp => {
                    selected = selected.saturating_sub(view_h);
                }
                KeyCode::PageDown => {
                    selected = (selected + view_h).min(models.len() - 1);
                }
                KeyCode::Home => {
                    selected = 0;
                }
            ...
            if selected >= scroll + view_h {
                scroll = selected.saturating_sub(view_h - 1);
                redraw = true;
            }
            let old_scroll = if old >= scroll && old < scroll + view_h {
                Some(old)
            } else {
                None
            };
            let new_scroll = if selected >= scroll && selected < scroll + view_h {
                Some(selected)
            } else {
                None
            };
...
fn draw_picker_view(
    ...
    let redraw_header = old_scroll_off.is_none_or(|os| os < scroll || os >= scroll + view_h);
    if redraw_header {
        let _ = execute!(
            stdout,
            Clear(ClearType::All),
...
fn patch_picker_lines(
    ...
    if old >= scroll && old < end {
        let _ = execute!(
            stdout,
            MoveTo(0, line_row(old)),
            Clear(ClearType::CurrentLine),
            SetForegroundColor(Color::White),
            Print(format!("  {}", models[old])),
            ResetColor,
            Print("\r\n")
        );
    }
    if selected >= scroll && selected < end {
        let _ = execute!(
            stdout,
            MoveTo(0, line_row(selected)),
            Clear(ClearType::CurrentLine),
            SetForegroundColor(Color::White),
            SetBackgroundColor(Color::DarkGrey),
            Print(format!("> {}", models[selected])),
            ResetColor,
            Print("\r\n")
        );
    }
...
fn list_models(default: &str) -> Vec<String> {
    ...
    if executor == "opencode" {
        let bin = which::which("opencode")
            .unwrap_or_else(|_| std::path::Path::new("opencode").to_path_buf());
...
fn run_project(project: &PathBuf, goal: &str, model_override: Option<&str>, interactive: bool) {
    ...
    eprintln!(
        "  artifacts: {}/artifacts/",
        project.join("tree/root").display()
    );
